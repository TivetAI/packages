...
use tivet_migrate::{SqlService, SqlServiceKind};
use tivet_service_manager::{CronConfig, Service, ServiceKind};
...
pub fn config(tivet_config: tivet_config::Config) -> Result<RunConfigData> {
	let server_config = tivet_config.server().map_err(|err| anyhow!("{err:?}"))?;
	...
	if server_config.tivet.auth.access_kind == tivet_config::config::tivet::AccessKind::Development
	{
		services.push(Service::new(
	...
	if server_config.nomad.is_some() && server_config.tivet.job_run.is_some() {
		services.push(Service::new(
			"job_gc",
			ServiceKind::Singleton,
	...
	if server_config.tivet.telemetry.enable {
		services.push(Service::new(
			"telemetry_beacon",
			ServiceKind::Cron(CronConfig {
