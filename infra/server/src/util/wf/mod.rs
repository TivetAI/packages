...
use clap::ValueEnum;
use indoc::indoc;
use tivet_pools::CrdbPool;
use tivet_term::console::style;
use uuid::Uuid;
...
pub async fn print_workflows(workflows: Vec<WorkflowRow>, pretty: bool) -> Result<()> {
	if workflows.is_empty() {
		tivet_term::status::success("No workflows found", "");
		return Ok(());
	}

	tivet_term::status::success("Workflows", workflows.len());
...
pub async fn print_history(
	...
	let Some(workflow) = wf_row else {
		tivet_term::status::success("No workflow found", "");
