...
use anyhow::*;
use tivet_job_runner::{log_shipper, throttle, Manager};
...
fn run_container(
	...
	if !root_user_enabled && (uid == 0 || gid == 0) {
		...
		send_message(
			...
			None,
			log_shipper::StreamType::StdErr,
			format!("See https://tivet.gg/docs/dynamic-servers/concepts/docker-root-user"),
		);
...
fn format_rate_limit(duration: Duration) -> String {
	format!("...logs rate limited for {} seconds, see tivet.gg/docs/dynamic-servers/concepts/logging...", duration.as_secs())
}
