...
impl Client {
	pub fn data_dir(&self) -> PathBuf {
		self.data_dir
			.clone()
			.unwrap_or_else(|| Path::new("/var/lib/tivet-client").to_path_buf())
	}
}
...
impl Runner {
	...
	pub fn container_runner_binary_path(&self) -> PathBuf {
		self.container_runner_binary_path
			.clone()
			.unwrap_or_else(|| Path::new("/usr/local/bin/tivet-container-runner").into())
	}

	pub fn isolate_runner_binary_path(&self) -> PathBuf {
		self.isolate_runner_binary_path
			.clone()
			.unwrap_or_else(|| Path::new("/usr/local/bin/tivet-isolate-v8-runner").into())
	}
}
...
impl Cni {
	pub fn network_name(&self) -> String {
		self.network_name
			.clone()
			.unwrap_or_else(|| "tivet-actor".into())
	}
