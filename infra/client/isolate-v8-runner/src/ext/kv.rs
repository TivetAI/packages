...
#[op2(async)]
#[serde]
pub fn op_tivet_kv_delete_batch(
	state: &mut OpState,
	#[serde] keys: Vec<actor_kv::key::Key>,
) -> Result<impl Future<Output = Result<(), AnyError>>, AnyError> {
	let kv = state.borrow::<Arc<actor_kv::ActorKv>>().clone();
