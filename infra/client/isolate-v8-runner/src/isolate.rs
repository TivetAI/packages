...
pub async fn run_inner(
	...
	let mut worker = MainWorker::try_bootstrap_from_options(
		...
		WorkerOptions {
			extensions: vec![
				ext::kv::tivet_kv::init_ops_and_esm(kv),
				ext::runtime::tivet_runtime::init_ops_and_esm(),
			],
...
fn handle_entrypoint(
	...
	// Get tivet ns
	let tivet_ns_module_id = mm
		.get_id(
			&"ext:tivet_runtime/90_tivet_ns.js",
			deno_core::RequestedModuleType::None,
		)
		.context("ns should be loaded")?;
	let tivet_g_ns = mm.get_module_namespace(scope, tivet_ns_module_id)?;
	let tivet_ns = v8::Local::new(scope, tivet_g_ns);

	// Get deep freeze function
	let deep_freeze_name = v8::String::new(scope, "deepFreeze").context("v8 primitive")?;
	let deep_freeze = tivet_ns
	...
	let frozen_ctx = deep_freeze
		.call(scope, tivet_ns.into(), &[ctx_export.into()])
