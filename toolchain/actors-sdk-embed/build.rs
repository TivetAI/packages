...
async fn main() -> Result<()> {
	...
	let output = tokio::process::Command::new("yarn")
		.arg("run")
		.arg("build")
		.arg("--filter=@tivet-gg/actor-manager")
	...
	// Build manager using Tivet build script (not using tsup/turbo because this includes custom
...
pub async fn build_backend_command_raw(opts: CommandOpts) -> Result<()> {
	...
	// Get JS utils
	let base = tivet_js_utils_embed::src_path(&data_dir.path().to_owned()).await?;
