...
async fn archive_oci_bundle(
	...
	// See umoci implementation: https://github.com/opencontainers/umoci/blob/312b2db3028f823443d6a74d86b05f65701b0d0e/oci/config/convert/runtime.go#L183
	{
		...
		// If you're you're the guy at Docker who decided to reimplement passwd in Go for funzies, please reconsider next time.
		{
			...
			if !allow_root {
				if uid == 0 {
					bail!("cannot run Docker container as root user (i.e. uid 0) for security. see https://tivet.gg/docs/dynamic-servers/concepts/docker-root-user")
				}
				if gid == 0 {
					bail!("cannot run Docker container as root group (i.e. gid 0) for security. see https://tivet.gg/docs/dynamic-servers/concepts/docker-root-user")
				}
			}
...
fn copy_container_to_rootfs(
	...
	let container_name = format!("tivet-game-{}", Uuid::new_v4());
