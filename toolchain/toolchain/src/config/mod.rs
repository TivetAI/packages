...
impl Config {
	pub async fn config_path(path: Option<&Path>) -> Result<PathBuf> {
		...
		let jsonc_path = path.join("tivet.jsonc");
		let json_path = path.join("tivet.json");

		let file_path = match (jsonc_path.exists(), json_path.exists()) {
			(true, true) => bail!("Both tivet.jsonc and tivet.json exist. Please remove one."),
			(false, false) => bail!("Neither tivet.jsonc nor tivet.json exist."),
