...
pub struct Meta {
	...
	/// If logged in to Tivet, this will include relevant information.
	///
...
impl Meta {
	pub fn cloud(&self) -> Result<&Cloud> {
		Ok(self
			.cloud
			.as_ref()
			.ok_or_else(|| errors::UserError::new("Not logged in. Please run `tivet login`."))?)
	}
...
#[derive(Serialize, Deserialize)]
pub struct Cloud {
	/// Tivet API endpoint to connect to.
