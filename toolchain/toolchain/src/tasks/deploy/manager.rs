...
pub async fn deploy(
	...
	let tags = HashMap::from([
		...
		("owner".to_string(), "tivet".to_string()),
	]);

	// Deploy manager
	let manager_script_path = tivet_actors_sdk_embed::dist_path(&paths::data_dir()?)
		.await?
		.join("manager")
		.join("index.js");
	...
	let actor = if let Some(actor) = res.actors.into_iter().next() {
		...
		// Choose a region that's closest to the core Tivet datacenter. This actor makes a lot of API
		// requests to Tivet, so we want to reduce that RTT.
