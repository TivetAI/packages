...
pub fn inquire_self_hosting() -> Result<Option<String>> {
	let using_cloud = inquire::Confirm::new("Are you using Tivet Cloud?")
		.with_default(true)
		.prompt()?;
...
pub async fn login(api_endpoint: Option<String>) -> Result<()> {
	...
	let api_endpoint = if let Some(e) = &api_endpoint {
		...
		// Fallback to main
		input_endpoint.unwrap_or_else(|| "https://api.tivet.gg".to_string())
	};
	...
	.await?;
	if output.signed_in {
		eprintln!("Already logged in. Log out with `tivet logout`.");
		return Ok(());
	}
