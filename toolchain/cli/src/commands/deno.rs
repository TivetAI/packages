...
use clap::Parser;
use serde::Serialize;
use toolchain::{errors, tivet_api::apis, tasks::get_bootstrap_data};
...
impl Opts {
	pub async fn execute(&self) -> Result<()> {
		...
		if self.populate_env {
			...
			cmd.env("TIVET_ENDPOINT", &ctx.api_endpoint)
				.env("TIVET_SERVICE_TOKEN", service_token.token)
				.env("TIVET_PROJECT", &ctx.project.name_id)
				.env("TIVET_ENVIRONMENT", &env.slug);
		}

		if let Result::Ok(path) = std::env::current_exe() {
			let final_path = std::fs::canonicalize(&path).unwrap_or(path);
			cmd.env("TIVET_CLI_PATH", final_path);
		}
