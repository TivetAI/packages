...
#[derive(Parser)]
#[clap(
	author = "Tivet Gaming, Inc. <developer@tivet.gg>",
	about = "https://tivet.gg/",
...
async fn report_error(err: anyhow::Error) {
	...
	util::telemetry::capture_event(
        ...
        Some(|event: &mut async_posthog::Event| {
            ...
            event.insert_prop("$sentry_url", format!("https://sentry.io/organizations/tivet-gaming/issues/?project=4508361147809792&query={event_id}"))?;
            Ok(())
        }),
    )
    .await;
