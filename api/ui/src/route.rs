...
impl Router {
	...
	pub async fn __inner(
		...
		config: tivet_config::Config,
		_pools: tivet_pools::Pools,
		_cache: tivet_cache::Cache,
		_ray_id: uuid::Uuid,
		...
		if !config.server()?.tivet.ui.enable() {
			return Ok(None);
		}
		...
		let content = tivet_hub_embed::get_file_content(path);
			...
			None => {
				...
				if path.ends_with(".html") || !path.contains('.') || path.contains("device.") {
					...
					let index_content = unwrap!(
						tivet_hub_embed::get_file_content("index.html"),
