...
// use tivet_identity::{model, output};
...
// 	fn http_client(&self, bearer_token: String) -> tivet_identity::ClientWrapper {
// 		tivet_identity::Config::builder()
...
// 		let token_claims = tivet_claims::decode(&identity_link_token).unwrap().unwrap();
...
// 	// Revoke the identity
// 	{
// 		let claims = tivet_claims::decode(&token).unwrap().unwrap();
