...
use tivet_convert::{ApiInto, ApiTryFrom};
...
pub async fn get(
	...
	let openapi_version = tivet_convert::cloud::version::config_to_openapi(
		ctx.op_ctx(),
...
pub async fn create(
	...
	let proto_config =
		tivet_convert::cloud::version::config_to_proto(ctx.op_ctx(), game_id, *body.config).await?;
