...
lazy_static! {
	static ref SERVICE_NAME: String =
		env::var("TIVET_SERVICE_NAME").unwrap_or_else(|_| "tivet".to_string());
	static ref SERVICE_INSTANCE: String =
		env::var("TIVET_SERVICE_INSTANCE").unwrap_or_else(|_| Uuid::new_v4().to_string());
	static ref SOURCE_HASH: String =
		env::var("TIVET_SOURCE_HASH").unwrap_or_else(|_| "unknown".to_string());
}
...
/// caches.
///
/// This will use the `TIVET_SOURCE_HASH` and fall back to the Git hash otherwise.
