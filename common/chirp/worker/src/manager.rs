...
use tivet_connection::Connection;
use tivet_operation::{prelude::err_code, OperationContext};
use tivet_pools::prelude::*;
use tivet_util::CleanExit;
use tokio::time;
use tracing::Instrument;
use types_proto::tivet::chirp;
	...
	W: Worker,
{
	...
	pub(crate) config: tivet_config::Config,
	pub(crate) pools: tivet_pools::Pools,
	...
	W: Worker,
{
	...
	async fn handle_raw_msg(
		...
		let (
			...
			WorkerKind::Consumer { .. } => match chirp::Message::decode(raw_msg_buf.as_slice()) {
				Ok(msg) => {
					// Calculate recv lag
					let recv_lag = (tivet_util::timestamp::now() as f64 - msg.ts as f64) / 1000.;
	...
	async fn handle_req_with_retry(
		...
		let mut backoff = tivet_util::Backoff::new(5, Some(3), 250, 100);
		loop {
			let res = self
				.worker
				.handle(req.op_ctx())
	...
	async fn consumer_ack(self: Arc<Self>, msg_meta: RedisMessageMeta) {
		// let mut backoff = tivet_util::Backoff::default();
		// loop {
		// 	if !backoff.tick().await {
