...
macro_rules! workers {
    ($($worker:ident),* $(,)?) => {
		...
		pub fn spawn_workers(shared_client: chirp_client::SharedClientHandle, config: tivet_config::Config, pools: tivet_pools::Pools, cache: tivet_cache::Cache, join_set: &mut tokio::task::JoinSet<GlobalResult<()>>) -> GlobalResult<()> {
