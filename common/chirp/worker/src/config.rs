...
impl Config {
	pub fn from_env(topic: &str) -> Result<Self, ManagerError> {
		let worker_kind = WorkerKind::Consumer {
			topic: topic.into(),
			group: tivet_env::service_name().to_string(),
		};

		Ok(Self {
			service_name: tivet_env::service_name().to_string(),
			worker_instance: tivet_env::service_instance().to_string(),
			worker_source_hash: tivet_env::source_hash().to_string(),
			worker_kind,
		})
	}
}
