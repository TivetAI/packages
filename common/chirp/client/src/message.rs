...
pub const MESSAGE_LOG_BUCKET_MS: i64 = tivet_util::duration::hours(1);
	...
	M: Message,
{
	...
	pub(crate) fn decode_inner(
		buf: &[u8],
	) -> Result<(types_proto::tivet::chirp::Message, Vec<TraceEntry>), ClientError> {
		// Decode the message and trace
		let message = <types_proto::tivet::chirp::Message as prost::Message>::decode(buf)
...
impl TryFrom<types_proto::tivet::chirp::TraceEntry> for TraceEntry {
	type Error = ClientError;

	fn try_from(value: types_proto::tivet::chirp::TraceEntry) -> Result<Self, ClientError> {
		Ok(TraceEntry {
