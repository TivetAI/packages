...
impl SharedClient {
	...
	pub fn from_env(pools: tivet_pools::Pools) -> Result<SharedClientHandle, ClientError> {
		Ok(SharedClient::new(
...
impl Client {
				...
				if M::HISTORY {
					...
					if rand::thread_rng().gen_bool(0.1) {
						pipe.cmd("ZREMRANGEBYSCORE")
							.arg(&history_key)
							.arg("-inf")
							.arg(tivet_util::timestamp::now() - ttl * 1000)
							.ignore();
					}
		...
		M: Message,
	{
		...
		let mut backoff = tivet_util::Backoff::default_infinite();
		loop {
	...
	/// of a `tivet_util::macros::select_with_timeout!` block:
	/// ```rust
	/// use tivet_util as util;
	///
...
impl TailAnchor {
	...
	pub fn is_valid(&self, ttl: i64) -> bool {
		self.start_time > tivet_util::timestamp::now() - ttl * 1000 - TAIL_ANCHOR_VALID_GRACE
	}
}
