...
///
/// ```
/// use tivet_identity::{Builder, Client, Config};
/// let raw_client =
///     Builder::dyn_https()
/// #     /*
	...
	R: aws_smithy_client::retry::NewRequestPolicy,
{
	...
	///   - [`identity_link_token(Option<String>)`](crate::output::PrepareGameLinkOutput::identity_link_token): Pass this to `tivet.api.identity#GetGameLink` to get the linking status. Valid for 15 minutes.
	///   - [`identity_link_url(Option<String>)`](crate::output::PrepareGameLinkOutput::identity_link_url): The URL that the user should visit to link their Tivet account.
...
pub mod fluent_builders {
	...
	///
	/// Begins the process for linking an identity with the Tivet Hub. # Importance of Linking Identities When an identity is created via `tivet.api.identity#SetupIdentity`, the identity is temporary and is not shared with other games the user plays. In order to make the identity permanent and synchronize the identity with other games, the identity must be linked with the hub. # Linking Process The linking process works by opening `identity_link_url` in a browser then polling `tivet.api.identity#GetGameLink` to wait for it to complete. This is designed to be as flexible as possible so `identity_link_url` can be opened on any device. For example, when playing a console game, the user can scan a QR code for `identity_link_url` to authenticate on their phone.
	#[derive(std::clone::Clone, std::fmt::Debug)]
	...
	///
	/// Prepares an avatar image upload. Complete upload with `tivet.api.identity#CompleteIdentityAvatarUpload`.
	#[derive(std::clone::Clone, std::fmt::Debug)]
	...
	///
	/// Gets or creates an identity. Passing an existing identity token in the body refreshes the token. # Temporary Accounts Until the identity is linked with the Tivet Hub (see `tivet.api.identity#PrepareGameLink`), this identity will be temporary but still behave like all other identities. This is intended to allow users to play the game without signing up while still having the benefits of having an account. When they are ready to save their account, they should be instructed to link their account (see `tivet.api.identity#PrepareGameLink`). # Storing Token `identity_token` should be stored in some form of persistent storage. The token should be read from storage and passed to `tivet.api.identity#SetupIdentity` every time the client starts.
	#[derive(std::clone::Clone, std::fmt::Debug)]
