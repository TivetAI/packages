...
pub struct WatchResponse {
	/// Index indicating the version of the data responded. Pas this to `tivet.common#WatchQuery` to block and wait for the next response.
...
pub struct GetGameLinkNewIdentity {
	/// See `tivet.api.identity#SetupIdentityOutput$identity_token`.
	pub identity_token: std::string::String,
	/// See `tivet.api.identity#SetupIdentityOutput$identity_token_expire_ts`.
	pub identity_token_expire_ts: chrono::DateTime<chrono::Utc>,
	/// See `tivet.api.identity#SetupIdentityOutput$identity`.
...
pub enum GlobalEventKind {
	/// `tivet.api.identity.common#GlobalEventKind` variant for chat messages. Received any time a message is sent to a chat the identity is in.
	ChatMessage(GlobalEventChatMessage),
	/// `tivet.api.identity.common#GlobalEventKind` variant for chat reads. Received any time the last read timestamp is set. Used to update the status of unread indicators on chats.
	ChatRead(GlobalEventChatRead),
	/// `tivet.api.identity.common#GlobalEventKind` variant for a chat thread being removed. Received any time the current identity is no longer able to access the given thread. This can happen if
	ChatThreadRemove(GlobalEventChatThreadRemove),
	/// `tivet.api.identity.common#GlobalEventKind` variant for identity updates. Received any time identity details are changed OR the identity switches.
	IdentityUpdate(GlobalEventIdentityUpdate),
	/// `tivet.api.identity.common#GlobalEventKind` variant for party updates. Received when the identity should be instructed to join a lobby. This needs to be implemented in conjunction with parties in order to force clients to join the same lobby as the party they're in.
	MatchmakerLobbyJoin(GlobalEventMatchmakerLobbyJoin),
	/// `tivet.api.identity.common#GlobalEventKind` variant for party updates. Received any time the identity joins a party, a party is updated, or when the identity leaves a party.
...
pub struct MatchmakerLobbyJoinInfoPlayer {
	/// Pass this token through the socket to the lobby server. The lobby server will validate this token with `tivet.api.matchmaker#PlayerConnected$player_token`.
	pub token: std::string::String,
}

/// A matchmaker lobby port. Configured by `tivet.cloud#LobbyGroupRuntimeDockerPort$label`.
#[derive(Clone, Debug, Deserialize, Serialize)]
...
pub enum ChatMessageBody {
	/// `tivet.chat#ChatMessageBody` variant for indicating a new chat was created.
	ChatCreate(ChatMessageBodyChatCreate),
	/// `tivet.chat#ChatMessageBody` variant for deleted messages.
	Deleted(ChatMessageBodyDeleted),
	/// `tivet.chat#ChatMessageBody` variant for indicating an identity joined the group.
	GroupJoin(ChatMessageBodyGroupJoin),
	/// `tivet.chat#ChatMessageBody` variant for indicating an identity left the group.
	GroupLeave(ChatMessageBodyGroupLeave),
	/// `tivet.chat#ChatMessageBody` variant for indicating an identity has been kicked from the group.
	GroupMemberKick(ChatMessageBodyGroupMemberKick),
	/// `tivet.chat#ChatMessageBody` variant for indicating an identity followed the identity.
	IdentityFollow(ChatMessageBodyIdentityFollow),
	/// `tivet.chat#ChatMessageBody` variant for indicating a change in the party's current activity.
	PartyActivityChange(ChatMessageBodyPartyActivityChange),
	/// `tivet.chat#ChatMessageBody` variant holding an a party invitation.
	PartyInvite(ChatMessageBodyPartyInvite),
	/// `tivet.chat#ChatMessageBody` variant for indicating an identity joined the party.
	PartyJoin(ChatMessageBodyPartyJoin),
	/// `tivet.chat#ChatMessageBody` variant for indicating an identity requesting to join your party.
	PartyJoinRequest(ChatMessageBodyPartyJoinRequest),
	/// `tivet.chat#ChatMessageBody` variant for indicating an identity left the party.
	PartyLeave(ChatMessageBodyPartyLeave),
	/// `tivet.chat#ChatMessageBody` variant for text messages. Sent by other identities using the chat interface.
...
pub struct ChatMessageBodyPartyInvite {
	...
	/// A party handle.
	pub party: std::option::Option<PartyHandle>,
	/// Pass to `tivet.api.party#GetPartyFromInvite$token` to view more information about the party. Pass to `tivet.api.party.common#JoinPartyInvite$token` to join the party.
...
#[serde(rename_all = "snake_case")]
pub enum ChatTopic {
	/// `tivet.chat#ChatTopic` variant for direct (identity to identity) chats.
	Direct(ChatTopicDirect),
	/// `tivet.chat#ChatTopic` variant for groups.
	Group(ChatTopicGroup),
	/// `tivet.chat#ChatTopic` variant for parties.
...
pub struct SetupIdentityRequest {
	/// Token returned from previous call to `tivet.api.identity#SetupIdentity`. If this token is invalid, a new identity will be returned.
...
pub struct PrepareGameLinkResponse {
	/// Pass this to `tivet.api.identity#GetGameLink` to get the linking status. Valid for 15 minutes.
	pub identity_link_token: std::string::String,
	/// The URL that the user should visit to link their Tivet account.
...
pub struct SetupIdentityResponse {
	/// Token used to authenticate the identity. Should be stored somewhere permanent. Pass this to `tivet.api.identity#SetupIdentity$existing_identity_token` next time `tivet.api.identity#SetupIdentity` is called. Token has a 90 day TTL. This means that if `tivet.api.identity#SetupIdentity` is not called again within 90 days, the token will no longer be valid. If this happens, the user can recover their account through the linking process (see `tivet.api.identity#PrepareGameLink`). This token should be stored locally and never sent to a server or another device. If this token is compromised, anyone with access to this token has control of the identity.
	pub identity_token: std::string::String,
