...
impl Builder {
	...
	pub fn build(self) -> Config {
		Config {
			auth: self
				.auth
				.or_else(|| std::env::var("TIVET_LOBBY_TOKEN").ok())
				.or_else(|| std::env::var("TIVET_CLIENT_TOKEN").ok()),
			uri: self
				.uri
				.or_else(|| std::env::var("TIVET_CF_VERIFICATION_API_URL").ok())
				.unwrap_or_else(|| "https://cf-verification.api.tivet.gg/v1".to_string()),
		}
	}
