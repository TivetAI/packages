...
pub enum JoinPartyInvite {
	/// A party invite alias. See `tivet.api.party.common#CreatePartyInviteConfig$alias` and `tivet.api.party#CreatePartyInvite$alias`.
	Alias(std::string::String),
	/// Requires the party publicity to this identity to be `tivet.party#PartyPublicityLevel$JOIN`.
	PartyId(std::string::String),
	/// A party invite token. See `tivet.api.party.common#CreatedInvite$token`.
...
pub struct CreatedInvite {
	/// An alias used to join a given party. This alias must be unique for all invites for your game. Pass this alias to `tivet.api.party.common#CreatedInvite$alias` to consume the invite.
...
pub struct CreatePartyPublicityConfig {
	/// Defaults to `tivet.party#PartyPublicityLevel$VIEW`.
	pub public: std::option::Option<PartyPublicityLevel>,
	/// Defaults to `tivet.party#PartyPublicityLevel$JOIN`.
...
pub struct JoinPartyRequest {
	...
	/// If the player is currently in the lobby, pass the token from `tivet.matchmaker#MatchmakerLobbyJoinInfoPlayer$token`. This will prevent issuing a new player token.
...
pub struct CreatePartyRequest {
	...
	/// If the player is currently in the lobby, pass the token from `tivet.matchmaker#MatchmakerLobbyJoinInfoPlayer$token`. This will prevent issuing a new player token and automatically set the party state to the player's current lobby.
	pub matchmaker_current_player_token: std::option::Option<std::string::String>,
}
