...
pub enum WorkflowError {
	...
	CreateSubscription(tivet_pools::prelude::nats::Error),

	#[error("failed to flush nats: {0}")]
	FlushNats(tivet_pools::prelude::nats::Error),
	...
	Redis {
		#[from]
		source: tivet_pools::prelude::redis::RedisError,
	},
	...
	pub(crate) fn deadline_ts(&self) -> Option<i64> {
		match self {
			...
			| WorkflowError::OperationTimeout(error_count) => {
				...
				let mut backoff =
					tivet_util::Backoff::new_at(8, None, RETRY_TIMEOUT_MS, 500, *error_count);
