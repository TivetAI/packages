...
impl TestCtx {
	pub async fn from_env(test_name: &str) -> TestCtx {
		...
		let config = tivet_config::Config::load::<String>(&[]).await.unwrap();
		let pools = tivet_pools::Pools::new(config.clone())
			.await
			.expect("failed to create pools");
		let shared_client = chirp_client::SharedClient::from_env(pools.clone())
			.expect("failed to create chirp client");
		let cache =
			tivet_cache::CacheInner::from_env(pools.clone()).expect("failed to create cache");
		...
		let op_ctx = tivet_operation::OperationContext::new(
...
impl TestCtx {
	...
	pub fn conn(&self) -> &tivet_connection::Connection {
		&self.conn
	}
