...
pub struct DatabaseCrdbNats {
	pool: PgPool,
	nats: NatsPool,
	sub: Mutex<Option<tivet_pools::prelude::nats::Subscriber>>,
}
...
impl DatabaseCrdbNats {
	...
	where
		...
		T: 'a,
	{
		let mut backoff = tivet_util::Backoff::new(4, None, QUERY_RETRY_MS, 50);
		let mut i = 0;
...
impl Database for DatabaseCrdbNats {
	...
	async fn fail_workflow(
		...
		if let Some(deadline_ts) = deadline_ts {
			if deadline_ts
				< tivet_util::timestamp::now() + worker::TICK_INTERVAL.as_millis() as i64 + 1
			{
				self.wake_worker();
			}
		}
