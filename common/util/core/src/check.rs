...
///
/// Will prevent domains from matching Tivet-specific domains if
/// `is_external` is true.
pub fn domain(config: &tivet_config::Config, s: impl AsRef<str>, is_external: bool) -> bool {
	let s = s.as_ref();
	...
	if is_external {
		if let Some(dns) = &server_config.tivet.dns {
