...
impl Service {
	...
	where
		F: Fn(tivet_config::Config, tivet_pools::Pools) -> Fut + Send + Sync + 'static,
...
pub async fn start(
	...
	services.push(Service::new(
		...
		ServiceKind::Core,
		|config, pools| {
			tivet_health_checks::run_standalone(tivet_health_checks::Config {
				config,
				pools: Some(pools),
			})
		},
	...
	services.push(Service::new(
		"metrics",
		ServiceKind::Core,
		|config, _pools| tivet_metrics::run_standalone(config),
	));
	...
	for service in services {
		...
		match service.kind.behavior() {
			ServiceBehavior::Service => {
				join_set
					.build_task()
					.name(&format!("tivet::service::{}", service.name))
					.spawn({
			...
			ServiceBehavior::Oneshot => {
				join_set
					.build_task()
					.name(&format!("tivet::oneoff::{}", service.name))
					.spawn({
						let config = config.clone();
						let pools = pools.clone();
						async move {
			...
			ServiceBehavior::Cron(cron_config) => {
				...
				if cron_config.run_immediately {
					...
					join_set
						.build_task()
						.name(&format!("tivet::cron_immediate::{}", service.name))
						.spawn({
							let config = config.clone();
							let pools = pools.clone();
							async move {
