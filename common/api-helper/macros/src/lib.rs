	...
	fn render(self) -> syn::Result<TokenStream2> {
		...
		let mounts = self
			...
			.map(|mount| {
				...
				quote! {
					.try_or_else(|| {
						tivet_operation::prelude::futures_util::FutureExt::boxed(
		...
		Ok(quote! {
			...
			impl Router {
				...
				) -> tivet_operation::prelude::GlobalResult<Option<Vec<u8>>> {
					use std::str::FromStr;
					...
					let res = match res {
						Ok(body) => {
							body.ok_or_else(|| {
								...
								tivet_operation::prelude::GlobalError::bad_request(
									tivet_operation::prelude::formatted_error::code::API_NOT_FOUND
								)
							})
						}
						Err(err) => Err(err),
...
impl Endpoint {
	fn render(self, cors_config: Option<syn::Expr>) -> syn::Result<TokenStream2> {
		...
		Ok(quote! {
			.try_or_else(|| tivet_operation::prelude::futures_util::FutureExt::boxed(tracing::Instrument::in_current_span(async {
				...
				if path_segments.next().is_none() {
					...
					match request.method() {
						#(#arms)*
						_ => {
							Err(tivet_operation::prelude::err_code!(
	...
	fn render(
		...
		let rate_limit = {
			...
			if let Some(rate_limit_rate) = self.args.iter().find(|arg| arg.label == "rate_limit") {
				...
				let buckets = if let Some(value) = args.get("buckets") {
					...
					let parsed_buckets = buckets
						...
						.map(|bucket| {
							...
							Ok(quote! {
								tivet_cache::RateLimitBucketConfig {
									count: #count,
				...
				quote! {
					tivet_cache::RateLimitConfig {
						key: format!("{}-{}", tivet_env::service_name(), #key),
						buckets: #buckets,
					}
				}
			} else {
				quote! {
					tivet_cache::RateLimitConfig {
						key: format!("{}{}", tivet_env::service_name(), #default_key),
		...
		Ok(quote! {
			&http::Method::#req_type => {
				...
				tokio::task::Builder::new()
					...
					.spawn(tivet_operation::prelude::futures_util::FutureExt::boxed(tracing::Instrument::in_current_span(async move {
				...
				let (response, http_status, err_code) = match ::tokio::time::timeout(timeout, fut).await {
					...
					Err(_) => {
						let err = tivet_operation::prelude::err_code!(API_REQUEST_TIMEOUT);
						let http_status = err.http_status();
						let err_code = err.code().unwrap_or("?").to_string();
						(Err(err), http_status, err_code)
					}
