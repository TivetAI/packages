...
use tivet_config::config::tivet::DnsProvider;
...
const MAX_ALLOWED_BODY_SIZE: u64 = tivet_util::file_size::gibibytes(10);
...
#[doc(hidden)]
impl __RouterConfig {
	pub fn new(config: &tivet_config::Config, uri: &hyper::Uri) -> GlobalResult<Self> {
		...
		let origin = tivet_util::url::to_string_without_slash(
			&config.server()?.tivet.api_public.public_origin(),
		);
...
pub async fn __with_ctx<A: auth::ApiAuth + Send>(
	...
	rate_limit_config: tivet_cache::RateLimitConfig,
) -> GlobalResult<Ctx<A>> {
	...
	let remote_address = if internal_endpoint {
		...
		} else if config.server()?.tivet.api_public.respect_forwarded_for() {
	...
	let svc_name = tivet_env::service_name().to_string();
	...
	let conn = tivet_connection::Connection::new(client, pools.clone(), cache.clone());
