...
impl ApiTryFrom<(&tivet_config::Config, backend::game::Game)> for models::GameHandle {
	type Error = GlobalError;

	// TODO: Replace this nasty fn with a bare fn
	fn api_try_from(
		(config, value): (&tivet_config::Config, backend::game::Game),
	) -> GlobalResult<Self> {
...
impl ApiTryFrom<::tier::types::Tier> for models::CloudRegionTier {
	...
	fn api_try_from(value: ::tier::types::Tier) -> GlobalResult<Self> {
		Ok(models::CloudRegionTier {
			...
			tivet_cores_numerator: value.tivet_cores_numerator.api_try_into()?,
			tivet_cores_denominator: value.tivet_cores_denominator.api_try_into()?,
