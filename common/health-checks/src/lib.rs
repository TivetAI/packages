...
#[derive(Clone)]
pub struct Config {
	pub config: tivet_config::Config,
	pub pools: Option<tivet_pools::Pools>,
}
...
pub async fn run_standalone(config: Config) -> GlobalResult<()> {
	...
	let host = config.config.server()?.tivet.health.host();
	let port = config.config.server()?.tivet.health.port();
...
mod status {
	...
	pub mod essential {
		...
		#[tracing::instrument(skip_all)]
		pub async fn route(pools: tivet_pools::Pools) -> StatusResponse {
