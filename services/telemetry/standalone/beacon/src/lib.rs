...
//
// Tivet is a powerful yet complicated system and these metrics help us support the open-source
// community. If you have questions or concerns about this data, please reach out to us on Discord:
// https://tivet.gg/discord
...
pub async fn run_from_env(
	...
	event.insert_prop(
		...
		json!({
			...
			"os": os_report(),
			"source_hash": tivet_env::source_hash(),
	...
	event.insert_prop(
		"$group_set",
		json!({
			"name": ctx.config().server()?.tivet.namespace,
		}),
	)?;
	events.push(event);
...
///
/// This helps Tivet diagnose crash reports to easily pinpoint if issues are
...
///
/// This helps Tivet diagnose crash reports to understand if specific features
/// are causing crashes.
fn get_config_data(ctx: &OperationContext<()>) -> GlobalResult<serde_json::Value> {
	...
	Ok(json!({
		"tivet": {
			"namespace": server_config.tivet.namespace,
			"auth_access_kind": format!("{:?}", server_config.tivet.auth.access_kind),
			"dns_enabled": server_config.tivet.dns.is_some(),
			"job_run_enabled": server_config.tivet.job_run.is_some(),
			"api_origin": server_config.tivet.api_public.public_origin(),
			"hub_origin": server_config.tivet.ui.public_origin(),
		},
	}))
}
...
///
/// This helps Tivet diagnose crash reports to understand if parts of the
