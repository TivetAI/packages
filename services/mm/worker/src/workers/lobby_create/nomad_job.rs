...
pub fn gen_lobby_docker_job(
	...
	let cpu = tier.tivet_cores_numerator as u64 * 1_000 / tier.tivet_cores_denominator as u64; // Millicore (1/1000 of a core)
	...
	let resources = Resources {
		...
		CPU: if tier.tivet_cores_numerator < tier.tivet_cores_denominator {
			Some((tier.cpu - util_job::TASK_CLEANUP_CPU as u32).try_into()?)
		} else {
			None
		},
		cores: if tier.tivet_cores_numerator >= tier.tivet_cores_denominator {
			Some((tier.tivet_cores_numerator / tier.tivet_cores_denominator) as i32)
		} else {
			None
		},
	...
	let mut env = runtime
		...
		.chain([(
			"TIVET_API_ENDPOINT".to_string(),
			util::url::to_string_without_slash(&config.server()?.tivet.api_public.public_origin()),
		)])
		.chain(
			// DEPRECATED:
			[
				("TIVET_CHAT_API_URL", "chat"),
				("TIVET_GROUP_API_URL", "group"),
				("TIVET_IDENTITY_API_URL", "identity"),
				("TIVET_KV_API_URL", "kv"),
				("TIVET_MATCHMAKER_API_URL", "matchmaker"),
			]
			.iter()
			.map(|(env, service)| {
				(
		...
		.chain(
			[
				...
				("TIVET_LOBBY_ID", template_env_var("NOMAD_META_LOBBY_ID")),
				("TIVET_TOKEN", template_env_var("NOMAD_META_LOBBY_TOKEN")),
				("TIVET_REGION_ID", template_env_var("NOMAD_META_REGION_ID")),
				(
					"TIVET_REGION_NAME",
				...
				("TIVET_CPU", cpu.to_string()),
				// Memory in bytes
				("TIVET_MEMORY", memory.to_string()),
				// Memory in bytes for oversubscription
				("TIVET_MEMORY_OVERSUBSCRIBE", memory_max.to_string()),
				// DEPRECATED:
				(
					"TIVET_LOBBY_TOKEN",
	...
	Ok(Job {
		...
		parameterized_job: Some(Box::new(ParameterizedJobConfig {
			...
			meta_optional: Some(vec!["tivet_test_id".into()]),
		})),
