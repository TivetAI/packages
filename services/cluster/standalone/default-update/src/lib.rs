...
pub async fn start_inner(
	...
	let tivet_config = &ctx.config().server()?.tivet;

	let cluster_configs = tivet_config.clusters();
...
async fn upsert_cluster(
	ctx: &StandaloneCtx,
	cluster_slug: &str,
	cluster_config: &config::tivet::Cluster,
) -> GlobalResult<()> {
	// Fetch cluster data
	let cluster_res = ctx
...
struct UpsertDatacenterArgs<'a> {
	cluster_id: Uuid,
	dc_slug: &'a str,
	dc_config: &'a config::tivet::Datacenter,
...
async fn upsert_datacenter(
	...
	let guard_public_hostname = match &dc_config.guard.public_hostname {
		Some(tivet_config::config::server::tivet::GuardPublicHostname::DnsParent(x)) => {
			Some(cluster::types::GuardPublicHostname::DnsParent(x.clone()))
		}
		Some(tivet_config::config::server::tivet::GuardPublicHostname::Static(x)) => {
