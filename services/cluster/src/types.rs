...
impl From<tivet_config::config::tivet::BuildDeliveryMethod> for BuildDeliveryMethod {
	fn from(value: tivet_config::config::tivet::BuildDeliveryMethod) -> BuildDeliveryMethod {
		match value {
			tivet_config::config::tivet::BuildDeliveryMethod::TrafficServer => {
				BuildDeliveryMethod::TrafficServer
			}
			tivet_config::config::tivet::BuildDeliveryMethod::S3Direct => {
...
/// See `tivet_config::config::server::tivet::GuardPublicHostname` for docs.
...
impl GuardPublicHostname {
	pub fn from_columns(
		...
		let gph = match (gph_dns_parent, gph_static) {
			...
			(None, None) => {
				if let Ok(domain_job) = config.server()?.tivet.domain_job() {
