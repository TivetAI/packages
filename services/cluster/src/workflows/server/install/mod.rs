...
use tivet_operation::prelude::proto::{self, backend::pkg::token};
use ssh2::Session;
...
async fn install_over_ssh_v2(ctx: &ActivityCtx, input: &InstallOverSshInputV2) -> GlobalResult<()> {
	...
	tokio::task::spawn_blocking(move || {
		...
		write_script(&sess, "tivet_install", &install_script)?;
		write_script(&sess, "tivet_hook", &hook_script)?;
		write_script(&sess, "tivet_initialize", &initialize_script)?;
		...
		let script = [
			"chmod +x /usr/bin/tivet_install.sh",
			"chmod +x /usr/bin/tivet_hook.sh",
			"chmod +x /usr/bin/tivet_initialize.sh",
			"/usr/bin/tivet_install.sh",
		]
		.join(" && ");
