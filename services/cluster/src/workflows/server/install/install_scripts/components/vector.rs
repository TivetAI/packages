	...
	match pool_type {
		PoolType::Pegboard | PoolType::PegboardIsolate => {
			config_json["sources"]["pegboard_manager"] = json!({
				"type": "file",
				"include": ["/var/lib/tivet-client/logs/*"]
			});
			...
			config_json["sources"]["pegboard_isolate_v8_runner"] = json!({
				"type": "file",
				"include": ["/var/lib/tivet-client/runner/logs/*"]
			});
			...
			config_json["sources"]["pegboard_container_runners"] = json!({
				"type": "file",
				"include": ["/var/lib/tivet-client/actors/*/logs/*"]
			});
			...
			config_json["transforms"]["pegboard_container_runner_add_meta"] = json!({
				...
				"source": formatdoc!(
					...
					.actor_id = parse_regex!(.file, r'/var/lib/tivet-client/actors/(?P<actor_id>[0-9a-fA-F-]+)/logs/').actor_id
