...
pub fn tunnel(
	...
	for TunnelService { name, .. } in TUNNEL_SERVICES {
		tcp_server_transports.insert(
			name.to_string(),
			ServerTransport {
				server_name: format!("{name}.tunnel.tivet.gg"),
	...
	instance(Instance {
		...
		dynamic_config: tunnel_dynamic_config(&config.server()?.tivet.tunnel.public_host),
...
pub async fn gg_static_config(config: &tivet_config::Config) -> GlobalResult<String> {
	let gg_config = &config.server()?.tivet.guard;

	let http_provider_endpoint = if let Some(api_traefik_provider_token) =
		&config.server()?.tivet.api_edge.traefik_provider_token
	{
		format!(
