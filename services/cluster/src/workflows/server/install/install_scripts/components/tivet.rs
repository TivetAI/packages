...
pub fn create_hook(tunnel_name: &str, initialize_immediately: bool) -> GlobalResult<String> {
	let mut script =
		include_str!("../files/tivet_create_hook.sh").replace("__TUNNEL_NAME__", tunnel_name);

	if initialize_immediately {
		script.push_str("systemctl start tivet_hook\n");
	}
...
pub fn fetch_info(server_token: &str) -> GlobalResult<String> {
	Ok(include_str!("../files/tivet_fetch_info.sh")
...
pub fn fetch_tunnel_tls(
	...
	let mut script = include_str!("../files/tivet_fetch_tunnel_tls.sh")
	...
	if initialize_immediately {
		...
		script.push_str(indoc!(
			"
			systemctl start tivet_fetch_tunnel_tls.timer
			systemctl start --no-block tivet_fetch_tunnel_tls.service
			"
		));
	}
...
pub fn fetch_gg_tls(
	...
	let mut script = include_str!("../files/tivet_fetch_gg_tls.sh")
		.replace("__TRAEFIK_INSTANCE_NAME__", traefik_instance_name)
		.replace("__SERVER_TOKEN__", server_token)
		.replace(
			"__TUNNEL_API_EDGE_API__",
	...
	if initialize_immediately {
		...
		script.push_str(indoc!(
			"
			systemctl start tivet_fetch_gg_tls.timer
			systemctl start --no-block tivet_fetch_gg_tls.service
			"
		));
	}
