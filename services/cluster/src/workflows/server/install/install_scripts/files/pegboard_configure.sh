...
cat << 'EOF' > /etc/tivet-client/config.json
{
	"client": {
		"cluster": {
...
#    flowing through *ALL* rules in the TIVET-ADMIN chain
#    (for game traffic) and the TIVET-INPUT chain (for ATS
#    traffic).
# 4. Run `iptables -L -v -t mangle` and validate that
#    packets are flowing through *ALL* the rules in 
#    TIVET-ADMIN and TIVET-INPUT.
...
for ipt in iptables ip6tables; do
	...
	add_ipt_chain "\$ipt" "mangle" "TIVET-FORWARD"
	add_ipt_rule "\$ipt" "mangle" "FORWARD" "-j TIVET-FORWARD"

	add_ipt_chain "\$ipt" "filter" "TIVET-INPUT"
	add_ipt_rule "\$ipt" "filter" "INPUT" "-j TIVET-INPUT"

	add_ipt_chain "\$ipt" "mangle" "TIVET-INPUT"
	add_ipt_rule "\$ipt" "mangle" "INPUT" "-j TIVET-INPUT"

	# MARK: Create GG TOS
	#
	# Sets the TOS to minimize delay if not already set.
    if ! "\$ipt" -t mangle -L "TIVET-TOS-GG" &>/dev/null; then
        "\$ipt" -t mangle -N "TIVET-TOS-GG"
        echo "Created \$ipt chain: TIVET-TOS-GG"
    else
        echo "Chain already exists in \$ipt: TIVET-TOS-GG"
    fi
	add_ipt_rule "\$ipt" "mangle" "TIVET-TOS-GG" "-m tos ! --tos 0x0 -j RETURN"
	add_ipt_rule "\$ipt" "mangle" "TIVET-TOS-GG" "-j TOS --set-tos 0x10"

	# VLAN only applicable to IPv4
	if [ "\$ipt" == "iptables" ]; then
		# MARK: GG TOS
		add_ipt_rule "\$ipt" "mangle" "TIVET-FORWARD" "-s __GG_VLAN_SUBNET__ -d \$SUBNET_VAR -j TIVET-TOS-GG"
		add_ipt_rule "\$ipt" "mangle" "TIVET-FORWARD" "-s \$SUBNET_VAR -d __GG_VLAN_SUBNET__ -j TIVET-TOS-GG"
		...
		add_ipt_rule "\$ipt" "mangle" "TIVET-INPUT" "-s __ATS_VLAN_SUBNET__ -j TOS --set-tos Maximize-Throughput"
		# Deprioritize traffic so game traffic takes priority
		add_ipt_rule "\$ipt" "filter" "TIVET-INPUT" "-s __ATS_VLAN_SUBNET__ -j MARK --set-mark 2"
	fi
...
cat << EOF > /opt/cni/config/tivet-actor.conflist
{
	"cniVersion": "0.4.0",
	"name": "tivet-actor",
	"plugins": [
...
ConditionPathExists=/etc/tivet-client/

[Service]
ExecStart=/usr/local/bin/tivet-client -c /etc/tivet-client/config.json
Restart=always
RestartSec=2
