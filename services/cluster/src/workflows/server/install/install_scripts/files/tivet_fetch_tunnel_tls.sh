...
# but we need the dirs to exist for the tivet_fetch_tunnel_tls.sh script to run before
...
# Write script
cat << 'EOF' > /usr/bin/tivet_fetch_tunnel_tls.sh
#!/usr/bin/env bash
set -eu -o pipefail
...
chmod +x /usr/bin/tivet_fetch_tunnel_tls.sh

# Create systemd service file
cat << 'EOF' > /etc/systemd/system/tivet_fetch_tunnel_tls.service
[Unit]
...
Group=root
Type=oneshot
ExecStart=/usr/bin/tivet_fetch_tunnel_tls.sh
...
cat << 'EOF' > /etc/systemd/system/tivet_fetch_tunnel_tls.timer
[Unit]
...
Unit=tivet_fetch_tunnel_tls.service
...
systemctl enable tivet_fetch_tunnel_tls.timer
systemctl enable tivet_fetch_tunnel_tls.service
