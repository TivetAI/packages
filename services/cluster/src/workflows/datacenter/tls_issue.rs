...
use tivet_config::config::TlsAcmeDirectory;
use tokio::task;
...
const ENCRYPT_EMAIL: &str = "letsencrypt@tivet.gg";
...
pub(crate) async fn cluster_datacenter_tls_issue(
	...
	let domain_main = unwrap!(
		ctx.config().server()?.tivet.dns()?.domain_main.clone(),
	...
	let domain_job = unwrap!(
		ctx.config().server()?.tivet.dns()?.domain_job.clone(),
...
async fn acme_account(config: &tivet_config::Config) -> GlobalResult<Account<MemoryPersist>> {
