...
async fn handle(
	...
	// Attempt to resolve the URL by the tivet.game subdomain or the custom CDN
	// domain
	let mut resolution = resolve_tivet_game_subdomain(&ctx, domain).await?;
...
/// Attempt to resolve from a tivet.game subdomain.
#[tracing::instrument]
async fn resolve_tivet_game_subdomain(
	ctx: &OperationContext<game::namespace_resolve_url::Request>,
	domain: &str,
